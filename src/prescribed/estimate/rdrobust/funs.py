...
class rdrobust_output:
    ...
    def __repr__(self):
        ...
        fw_r = 14
        print(
            "Number of Observations:".ljust(fw), str(self.N[0] + self.N[1]).rjust(fw_r)
        )
        ...
        print(
            ...
            "t-stat".rjust(fw_c),
            "P>|t|".rjust(fw_c),
            (str(self.level) + "% CI").center(fw_ci),
        )
        print("-" * 73)
        ...
        for j in method_id:
            if j == 0:
                print(
                    ...
                    (
                        "["
                        + str(round(self.ci.iloc[j, 0].item(), n_dec))
                        + ", "
                        + str(round(self.ci.iloc[j, 1].item(), n_dec))
                        + "]"
                    ).rjust(fw_ci),
                )
            else:
...
class rdbwselect_output:
    def __init__(self, bws, bwselect, bw_list, kernel, p, q, c, N, M, vce, masspoints):
        self.bws = bws
        self.bwselect = bwselect
        ...
        for pos in range(n):
            ...
            if Z is not None:
                for i in range(dZ):
                    ...
                    res[pos, 1 + dT + i] = np.sqrt(Ji / (Ji + 1)) * (
                        Z[pos, i] - Z_J / Ji
                    )
    else:
        if vce == "hc0":
...
def rdrobust_bw(
    ...
    if vce == "hc0" or vce == "hc1" or vce == "hc2" or vce == "hc3":
        ...
        if vce == "hc2" or vce == "hc3":
            hii = np.sum(
                np.matmul(R_V, invG_V) * (R_V * eW.reshape(-1, 1)), axis=1
            ).reshape(-1, 1)

    res_V = rdrobust_res(
        eX, eY, eT, eZ, predicts_V, hii, vce, nnmatch, dups_V, dupsid_V, o + 1
    )
    ...
    if scale > 0:
        ...
        if vce == "hc0" or vce == "hc1" or vce == "hc2" or vce == "hc3":
            ...
            if vce == "hc2" or vce == "hc3":
                hii = np.sum(
                    np.matmul(R_B, invG_B) * (R_B * eW.reshape(-1, 1)), axis=1
                ).reshape(-1, 1)

        res_B = rdrobust_res(
            eX, eY, eT, eZ, predicts_B, hii, vce, nnmatch, dups_B, dupsid_B, o_B + 1
        )
