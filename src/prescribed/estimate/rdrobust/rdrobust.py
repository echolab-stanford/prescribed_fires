...
def rdrobust(
    ...
    if p is None and deriv is not None:
        p = deriv + 1
    if p is None:
        p = 1
    ...
    if q is None:
        ...
        raise Exception(
            "Polynomial order (for bias correction) q incorrectly specified"
        )
    ...
    if bwselect not in bw_list:
        ...
        raise Exception(
            "bwselect options IK, CCT and CV have been depricated. Please see help for new options"
        )
    ...
    if covs is not None:
        try:
            covs_names = list(covs.columns)
        except:
            covs_names = ["z" + str(i + 1) for i in range(ncol(covs))]
    ...
    if N < 20:
        print(
            "Not enough observations to perform bandwidth calculations. Estimates computed using entire sample"
        )
    ...
    Q_q_l = (
        (R_p_l * W_h_l).T
        - np.matmul(
            h_l ** (p + 1) * np.matmul(L_l, (e_p1).T),
            ((np.matmul(invG_q_l, R_q_l.T)).T * W_b_l).T,
        )
    ).T
    Q_q_r = (
        (R_p_r * W_h_r).T
        - np.matmul(
            h_r ** (p + 1) * np.matmul(L_r, (e_p1).T),
            ((np.matmul(invG_q_r, R_q_r.T)).T * W_b_r).T,
        )
    ).T
    D_l = eY_l.copy()
    D_r = eY_r.copy()
    ...
    if covs is None:
        ...
        if fuzzy is None:
            ...
            tau_Y_cl = float(
                np.matmul(
                    scalepar * np.math.factorial(deriv) * s_Y.T,
            ...
            tau_Y_cl_l = float(
                np.matmul(
                    scalepar * np.math.factorial(deriv) * s_Y.T,
                    np.append(beta_p_l[deriv, 0], beta_p_l[deriv, colsZ]).reshape(
                        -1, 1
                    ),
                )
            )
            ...
            tau_Y_cl_r = float(
                np.matmul(
                    scalepar * np.math.factorial(deriv) * s_Y.T,
                    np.append(beta_p_r[deriv, 0], beta_p_r[deriv, colsZ]).reshape(
                        -1, 1
                    ),
                )
            )
            ...
            tau_Y_bc_l = float(
                np.matmul(
                    scalepar * np.math.factorial(deriv) * s_Y.T,
                    np.append(beta_bc_l[deriv, 0], beta_bc_l[deriv, colsZ]).reshape(
                        -1, 1
                    ),
                )
            )
            ...
            tau_Y_bc_r = float(
                np.matmul(
                    scalepar * np.math.factorial(deriv) * s_Y.T,
                    np.append(beta_bc_r[deriv, 0], beta_bc_r[deriv, colsZ]).reshape(
                        -1, 1
                    ),
                )
            )
            ...
            tau_T_cl_l = float(
                np.matmul(
                    np.math.factorial(deriv) * s_T.T,
                    np.append(beta_p_l[deriv, 1], beta_p_l[deriv, colsZ]).reshape(
                        -1, 1
                    ),
                )
            )
            ...
            tau_T_cl_r = float(
                np.matmul(
                    np.math.factorial(deriv) * s_T.T,
                    np.append(beta_p_r[deriv, 1], beta_p_r[deriv, colsZ]).reshape(
                        -1, 1
                    ),
                )
            )
            ...
            tau_T_bc_r = float(
                np.matmul(
                    np.math.factorial(deriv) * s_T.T,
                    np.append(beta_bc_r[deriv, 1], beta_bc_r[deriv, colsZ]).reshape(
                        -1, 1
                    ),
                )
            )
            ...
            s_Y = np.hstack(
                [
                    s_Y,
                    -(1 / tau_T_cl) * gamma_p[:, 0]
                    + (tau_Y_cl / tau_T_cl**2) * gamma_p[:, 1],
                ]
            )
    ...
    V_Y_cl_l = np.matmul(
        invG_p_l,
        np.matmul(rdrobust_vce(dT + dZ, s_Y, R_p_l * W_h_l, res_h_l, eC_l), invG_p_l),
    )
    V_Y_cl_r = np.matmul(
        invG_p_r,
        np.matmul(rdrobust_vce(dT + dZ, s_Y, R_p_r * W_h_r, res_h_r, eC_r), invG_p_r),
    )
    V_Y_rb_l = np.matmul(
        invG_p_l, np.matmul(rdrobust_vce(dT + dZ, s_Y, Q_q_l, res_b_l, eC_l), invG_p_l)
    )
    V_Y_rb_r = np.matmul(
        invG_p_r, np.matmul(rdrobust_vce(dT + dZ, s_Y, Q_q_r, res_b_r, eC_r), invG_p_r)
    )
    V_tau_cl = (
        scalepar**2
        * np.math.factorial(deriv) ** 2
        * (V_Y_cl_l + V_Y_cl_r)[deriv, deriv]
    )
    V_tau_rb = (
        scalepar**2
        * np.math.factorial(deriv) ** 2
        * (V_Y_rb_l + V_Y_rb_r)[deriv, deriv]
    )
    ...
    Estimate = pd.DataFrame(
        np.array([tau_cl, tau_bc, se_tau_cl, se_tau_rb]).reshape(1, -1),
        columns=["tau.us", "tau.bc", "se.us", "se.rb"],
        index=pd.Index(["Estimate"]),
    )
    bws = pd.DataFrame(
        np.array([[h_l, h_r], [b_l, b_r]]),
        columns=["left", "right"],
        index=pd.Index(["h", "b"]),
    )
    label = ["Conventional", "Bias-Corrected", "Robust"]
    coef = pd.DataFrame(tau, columns=["Coeff"], index=pd.Index(label))
    se = pd.DataFrame(se, columns=["Std. Err."], index=pd.Index(label))
    t = pd.DataFrame(t, columns=["t-stat."], index=pd.Index(label))
    pv = pd.DataFrame(pv, columns=["P>|t|"], index=pd.Index(label))
    ci = pd.DataFrame(
        ci,
    ...
    return rdrobust_output(
        ...
        [N_b_l, N_b_r],
        [M_l, M_r],
        [tau_Y_cl_l, tau_Y_cl_r],
        [tau_Y_bc_l, tau_Y_bc_r],
        c,
        p,
        q,
        [bias_l, bias_r],
