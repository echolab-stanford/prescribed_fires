...
Taken from: https://github.com/rdpackages/rdrobust/tree/master/Python
"""
...
def rdplot(
    ...
    if kernel == "triangular" or kernel == "tri":
    ...
    B_es_hat_dw = np.array(
        [
            ((c - x_min) ** 2 / (12 * n)) * np.sum(mu1_hat_l**2),
            ((x_max - c) ** 2 / (12 * n)) * np.sum(mu1_hat_r**2),
        ]
    )
    V_es_hat_dw = np.array(
        [
            (0.5 / (c - x_min)) * sum(dxi_l * dyi_l**2),
            (0.5 / (x_max - c)) * sum(dxi_r * dyi_r**2),
        ]
    )
    V_es_chk_dw = np.array(
        [
            (1 / (c - x_min)) * sum(dxi_l * sigma2_hat_l_bar),
    ...
    B_qs_hat_dw = np.array(
        [
            (n_l**2 / (24 * n)) * sum(dxi_l**2 * mu1_i_hat_l**2),
            (n_r**2 / (24 * n)) * sum(dxi_r**2 * mu1_i_hat_r**2),
        ]
    )
    V_qs_hat_dw = np.array(
        [(1 / (2 * n_l)) * sum(dyi_l**2), (1 / (2 * n_r)) * sum(dyi_r**2)]
    )
    V_qs_chk_dw = np.array(
        [(1 / n_l) * sum(sigma2_hat_l), (1 / n_r) * sum(sigma2_hat_r)]
    )
    J_qs_hat_dw = J_fun(B_qs_hat_dw, V_qs_hat_dw)
    J_qs_chk_dw = J_fun(B_qs_hat_dw, V_qs_chk_dw)

    J_es_hat_mv = np.array(
        [
            np.ceil((var_y_l / V_es_hat_dw[0]) * (n / np.log(n) ** 2)),
            np.ceil((var_y_r / V_es_hat_dw[1]) * (n / np.log(n) ** 2)),
        ]
    )
    J_es_chk_mv = np.array(
        [
            np.ceil((var_y_l / V_es_chk_dw[0]) * (n / np.log(n) ** 2)),
            np.ceil((var_y_r / V_es_chk_dw[1]) * (n / np.log(n) ** 2)),
        ]
    )
    J_qs_hat_mv = np.array(
        [
            np.ceil((var_y_l / V_qs_hat_dw[0]) * (n / np.log(n) ** 2)),
            np.ceil((var_y_r / V_qs_hat_dw[1]) * (n / np.log(n) ** 2)),
        ]
    )
    J_qs_chk_mv = np.array(
        [
            np.ceil((var_y_l / V_qs_chk_dw[0]) * (n / np.log(n) ** 2)),
            np.ceil((var_y_r / V_qs_chk_dw[1]) * (n / np.log(n) ** 2)),
        ]
    )
    ...
    if not np.isnan(J_star_l):
        J_star_l = int(J_star_l)
    J_star_r = scale_r * J_star_orig[1]
    if not np.isnan(J_star_r):
        J_star_r = int(J_star_r)

    if nbins is not None:
        if not np.isnan(J_star_l):
            J_star_l = int(nbins_l)
        if not np.isnan(J_star_r):
            J_star_r = int(nbins_r)
    ...
    bin_x_r[bin_x_r == J_star_r] = (
        J_star_r - 1
    )  # to mimick R behaviour of FindInterval function in R

    aux_l = pd.DataFrame({"bin_x_l": bin_x_l, "y_l": y_l, "x_l": x_l})
    rdplot_l = (
        aux_l.groupby("bin_x_l").agg({"y_l": "mean", "x_l": "mean"}).reset_index()
    )
    ...
    rdplot_r = (
        aux_r.groupby("bin_x_r").agg({"y_r": "mean", "x_r": "mean"}).reset_index()
    )
    ...
    if covs is not None and covs_eval == "mean":
        ...
        rdplot_mean_y_l = (
            aux_l.groupby("bin_x_l").agg({"yhatZ_l": "mean"})["yhatZ_l"].values
        )
        ...
        rdplot_mean_y_r = (
            aux_r.groupby("bin_x_r").agg({"yhatZ_r": "mean"})["yhatZ_r"].values
        )

    t_ind_l = np.arange(J_star_l)
    t_ind_r = np.arange(J_star_r)

    rdplot_mean_bin_l = np.mean(
        np.column_stack((jumps_l[t_ind_l], jumps_l[t_ind_l + 1])), axis=1
    )
    rdplot_mean_bin_r = np.mean(
        np.column_stack((jumps_r[t_ind_r], jumps_r[t_ind_r + 1])), axis=1
    )
    ...
    aux_l = (
        pd.DataFrame({"bin": -bin_x_l, "y": y_l})
        .groupby("bin")
        .agg({"y": ["count", "std"]})
    )
    aux_r = (
        pd.DataFrame({"bin": bin_x_r, "y": y_r})
        .groupby("bin")
        .agg({"y": ["count", "std"]})
    )
    ...
    if not hide:
        ...
        if plot:
            temp_plot = (
                ...
                + geom_point(
                    data=data_bins,
                    mapping=aes(x=rdplot_mean_bin, y=rdplot_mean_y),
                    color=col_dots,
                    na_rm=True,
                )
                + geom_line(
                    data=data_poly,
                    mapping=aes(x=x_plot_l, y=y_hat_l),
                    color=col_lines,
                    na_rm=True,
                )
                + geom_line(
                    data=data_poly,
                    mapping=aes(x=x_plot_r, y=y_hat_r),
                    color=col_lines,
                    na_rm=True,
                )
            )
            ...
            if flag_no_ci == False:
                temp_plot = temp_plot + geom_errorbar(
                    data=data_bins,
                    mapping=aes(
                        x=rdplot_mean_bin, ymin=rdplot_cil_bin, ymax=rdplot_cir_bin
                    ),
                )
            if shade:
                temp_plot = temp_plot + geom_ribbon(
                    data=data_bins,
                    mapping=aes(
                        x=rdplot_mean_bin, ymin=rdplot_cil_bin, ymax=rdplot_cir_bin
                    ),
    ...
    rdplot_min_bin = np.concatenate(
        (
            rdplot_min_bin_l[np.flip(-rdplot_bin_l) - 1],
            rdplot_min_bin_r[rdplot_bin_r - 1],
        )
    )
    rdplot_max_bin = np.concatenate(
        (
            rdplot_max_bin_l[np.flip(-rdplot_bin_l) - 1],
            rdplot_max_bin_r[rdplot_bin_r - 1],
        )
    )
    ...
    coef = pd.DataFrame(
        {"Left": gamma_p1_l.reshape(-1), "Right": gamma_p1_r.reshape(-1)}
    )
